...
module Controller (
    ...
    //sort type
    wire TYPE_R       = ADD | SUB | XOR | SLL | SRL | SRA | AND | OR | SLT | SLTU | MUL | MULH | MULHU | DIV | DIVU | REM | REMU;
    wire TYPE_I_ALU   = ADDI | ORI | ANDI | XORI | SLTI | SLTIU;
    wire TYPE_I_SHIFT = SRLI | SLLI | SRAI;
    wire TYPE_LOAD    = LW | LB | LBU | LH | LHU;
    wire TYPE_STORE   = SB | SH | SW;
    wire TYPE_BRANCH  = BEQ | BNE | BLT | BLTU | BGE | BGEU;
 
    // npc_op
    wire NPC_OP_BRA = TYPE_BRANCH;
    wire NPC_OP_JALR = JALR;
    wire NPC_OP_JMP = JAL;
    wire NPC_OP_PC4 = !NPC_OP_BRA & !NPC_OP_JALR & !NPC_OP_JMP;
    
    // rf_we
    wire RF_OP_WE = TYPE_R | TYPE_I_ALU | TYPE_I_SHIFT | TYPE_LOAD | JALR | LUI | AUIPC | JAL;
    
    // rf_wsel
    wire WB_OP_ALU = TYPE_R | TYPE_I_ALU | TYPE_I_SHIFT | AUIPC;
    wire WB_OP_RAM = TYPE_LOAD;
    wire WB_OP_PC4 = JALR | JAL;
    wire WB_OP_EXT = LUI;
    
    // sext_op
    wire EXT_OP_I = TYPE_I_ALU | TYPE_LOAD | JALR;
    wire EXT_OP_S = TYPE_STORE;
    wire EXT_OP_B = TYPE_BRANCH;
    wire EXT_OP_U = LUI | AUIPC;
    wire EXT_OP_J = JAL;
    wire EXT_OP_SHAMT = TYPE_I_SHIFT;
    
    // alu_op
    wire ALU_OP_ADD   = ADD | ADDI | TYPE_LOAD | TYPE_STORE | JALR | AUIPC;
    wire ALU_OP_SUB   = SUB;
    ...
    // alua_sel
    wire ALU_A_SEL_RS1 = TYPE_R | TYPE_I_ALU | TYPE_I_SHIFT | TYPE_LOAD | TYPE_STORE | TYPE_BRANCH | JALR;
    wire ALU_A_SEL_PC  = AUIPC;
                        
    // alub_sel
    wire ALU_B_SEL_RS2 = TYPE_R | TYPE_BRANCH;
    wire ALU_B_SEL_EXT = TYPE_I_ALU | TYPE_I_SHIFT | TYPE_LOAD | TYPE_STORE | JALR | AUIPC;
